...
public partial class MainWindow
{
    ...
    {
        TemperatureSlider.Background = new LinearGradientBrush(new GradientStopCollection
        {
        ...
        TintSlider.Background = new LinearGradientBrush(new GradientStopCollection
        {
    ...
    private byte[] RenderPixels(byte[] source, int width, int height, double[] v, double baseTemp, double baseTint, int maxWidth, CancellationToken token)
    {
        ...
        Parallel.For(0, outHeight, new ParallelOptions { CancellationToken = token }, y =>
        {
            ...
            for (int x = 0; x < outWidth; x++)
            {
                ...
                double shadowMask = Math.Clamp(1 - luma * 2, 0, 1), highlightMask = Math.Clamp((luma - .5) * 2, 0, 1);
                double whiteMask = Math.Clamp((luma - .7) / .3, 0, 1), blackMask = Math.Clamp((.3 - luma) / .3, 0, 1);
                ...
                r += temperature * .10; b -= temperature * .10;
                r += tint * .03; g -= tint * .03;
